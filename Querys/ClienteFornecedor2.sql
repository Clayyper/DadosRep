select distinct pes.codigopessoa as [Código],pes.nomepessoa as [Razão Social],dpessju.cnpj as [CNPJ],contab.codcompletoconta as [Conta Contábil Cliente],CONVERT(varchar(10), dcli.datacadastramento, 103) as [Data Cadastro Cliente],
contab2.codcompletoconta as [Conta Contábil Fornecedor],CONVERT(varchar(10), dforn.datacadastramento, 103) as [Data Cadastro Fornecedor],
...
left join
(select * from [NG_Contabil]..[ctb_contacontabil])as contab2 on dforn.idcontacontabil=contab2.idcontacontabil
left join
